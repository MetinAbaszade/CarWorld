    ...
    return (
        <Link to={`/car/${car.id}`}>
            <div className="m-3 w-72 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                ...
                <div className='p-3'>
                    <p className="text-xl mb-1 font-semibold text-slate-950">{car.price.toLocaleString('de-DE')} {car.currency}</p>
                    <h5 className="text-lg tracking-tight text-slate-950">{car.brand} {car.model}</h5>
                    <p className=" text-base font-normal text-slate-950">{car.releaseYear}, {car.engineVolume}L, {car.mileage.toLocaleString('de-DE')} {car.mileageType}</p>
